...
{field.type="id";delete field.source;}});}},this);_oldMetadataSet.apply(this,arguments);};app.data.sync=_.wrap(app.data.sync,function(_super,method,model,options){options=options||{};options.params=_.extend(options.params||{},{erased_fields:true});return _super.call(app.data,method,model,options);});})(SUGAR.App);
...
(function(app){app.events.on('app:init',function(){app.plugins.register('CssLoader',['layout','view','field'],{onAttach:function(){this.loadCss();},loadCss:function(cssFiles){var $previouslyAdded;_.each(cssFiles||this.css,function(file){var $link;if(!this.isCssLoaded(file)){if(file.indexOf('.css')===-1){file=file+'.css';}
file+='?v='+app.metadata.getHash();$link=$('<link>',{href:file,type:'text/css',rel:'stylesheet'});if($previouslyAdded){$previouslyAdded.after($link);}else{$link.prependTo(document.head);}
...
/* End of File custom/include/javascript/sugar7/plugins/CssLoader.js */
