...
function updatePacketContacts($contacts_ids, $env_rec_id) {
    ...
    if ($document_packet->load_relationship('dp_doucumentspackets_contacts')) {
        $contacts = $document_packet->dp_doucumentspackets_contacts->getBeans();
        foreach ($contacts as $contact) {
            $document_packet->dp_doucumentspackets_contacts->delete($contact->id);
        }
...
function updatePacketDocuments($document_ids, $env_rec_id) {
    ...
    $document_packet = BeanFactory::getBean('DP_DoucumentsPackets', $env_rec_id);
    if ($document_packet->load_relationship('dp_doucumentspackets_documents')) {
        $documents = $document_packet->dp_doucumentspackets_documents->getBeans();
        foreach ($documents as $document) {
            $document_packet->dp_doucumentspackets_documents->delete($document->id);
        }
