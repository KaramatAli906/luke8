<?php
    ...
    foreach ($modules as $module_key => $module) {
        ...
        $table_name = strtolower($module_key);
        $query = " SELECT dp.* from dp_doucumentspackets dp JOIN $table_name ON  dp.parent_id = $table_name.id";
        $count_query = " SELECT count(*) count from dp_doucumentspackets dp JOIN $table_name ON  dp.parent_id = $table_name.id";
        $where = " WHERE dp.parent_type !='' AND dp.deleted=0 AND $table_name.deleted=0";

        if (!empty($select_module)) {
            $module = $select_module;
            $where .= " AND dp.parent_type = '$module'";
        }
        if (!empty($track_date_from) && !empty($track_date_to)) {
            ...
            $where .= " AND (dp.date_entered between '$track_date_from 00:00:00' AND '$track_date_to 23:59:00')";
        } else if (!empty($track_date_from)) {
            $track_date_from = date("Y-m-d", strtotime($track_date_from));
            $where .= " AND dp.date_entered >= '$track_date_from 00:00:00'";
        } else if (!empty($track_date_to)) {
            $track_date_to = date("Y-m-d", strtotime($track_date_to));
            $where .= " AND dp.date_entered <= '$track_date_to 23:59:00'";
        }
        if (!empty($select_status[0]) && count($select_status) > 0) {
            $select_status_a = implode("','", $select_status);
            $select_status_a = "'" . $select_status_a . "'";
            $where .= " AND dp.packetstatus IN ($select_status_a)";
        }

        if (!$current_user->is_admin) {
            $where .= " AND dp.assigned_user_id = '$current_user->id'";
        }

        $count_result = $db->query($count_query . $where);
        $count_result = $db->fetchByAssoc($count_result);
        $total += $count_result['count'];

        if ($module_key != 'm_CAMS') {
            $where.=" UNION ";
        }
    ...
    $main_query .= " ORDER BY $sort_by $order LIMIT $limit OFFSET $offset";
    ...
    while ($row = $db->fetchByAssoc($result)) {
        ...
        while ($related_contact = $db->fetchByAssoc($related_contacts_result)) {
            if ($related_contact['dp_doucumentspackets_contactscontacts_idb'] != $row['parent_id']) {
                $total_contacts++;
