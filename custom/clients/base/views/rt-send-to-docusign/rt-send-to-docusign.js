({
    ...
    events: {
        ...
        'change [name="signed_attachment_type"]': 'copyAttachmentType',
    },
    copyAttachmentType: function ()
    {
        var attachment_type = $('[name="signed_attachment_type"]').val();
        var list = app.lang.getAppListStrings("document_template_type_dom");
        if (!_.isEmpty((list[attachment_type])))
        {
            $("#signed_attachment_name").val(list[attachment_type]);
        } else {
            $("#signed_attachment_name").val($('[name="signed_attachment_type"]').val());
        }
    },
    loadData: function (options) {
        var ParentID = this.context.get('ParentID');
        var Module = this.context.get('Module');
        var PdfDocID = this.context.get('PdfDocID');
        var PdfDocName = this.context.get('PdfDocName');

        var parenturl = window.location.href;
        var authtoken = app.api.getOAuthToken();
    ...
    populateInitials: function (contact_id, initial) {
        ...
        $(".send_docs_main .span12.controls-one").css("padding-bottom", "10px");
    },
    ...
    setDocumentsType: function () {
        ...
        if (this.documents_type == 'Sugar Attachments') {
            ...
            $(".select_initial").hide();
            $(".uploadfile").hide();
            $(".send_docs_main .attached_contacts .select2-container").css("width", "100%");
            $(".send_docs_main #contacts_grid input").css("width", "100%");
        } else if (this.documents_type == 'Upload From Local') {
            this.documents_type = 'Sugar Attachments';
            $('#docusign_templates').hide();
            $('#docusign_templates').hide();
            ...
            $(".send_docs_main .attached_contacts .select2-container").css("width", "68%");
            $(".send_docs_main #contacts_grid .contact_row input").css("width", "68%");
            $("dev.select_initial").show();
        }
    },
    SendToDocuSign: function () {
        ...
        var filepath = $('[name="uploadfile"]').val();
        var document_name_return = ""
        var attachment_id = "";
        if (filepath != "") {
            var url = encodeURI(this.sugarUrl.rootURL + 'index.php?entryPoint=createMvAttachment');
            var filedata = $('[name="uploadfile"]').prop("files")[0];
            var form_data = new FormData(this);
            form_data.append('pdfFile', filedata);
            form_data.append('parent_id', this.parent_id);
            form_data.append('parent_module', this.module);
            $.ajax({
                type: 'POST',
                url: url,
                data: form_data,
                ...
                success: function (data) {
                    ...
                    $('[name="attacheddocuments_c"]').val(data.id); //selected document name
                    document_name_return = data.name;
                }
            });
        }
        ...
        if (filepath != "") {
            documentname = document_name_return;
        } else {
        ...
        $('.docusign_templates').removeClass('error');
        if ((contacts.length == 0) && (documents.length == 0 && this.documents_type == 'Sugar Attachments')) {
            ...
            $("#maindiv").show();
            $("#record").show();
            $('.docusign_templates').addClass('error');
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#myIframe").show();
                    if (attachment_id != "") {
                        url = encodeURI(sugarUrl.apiRootURL + "/mv_Attachments/" + attachment_id);
                        $.ajax({
                            type: 'DELETE',
                            url: url,
                            headers: {
		...
		var windowHeight = $(window).height();
                    ...
                    var footerheight = $('.table-row').height();
                    var bottomBoxHeight = windowHeight - menuHeight - footerheight;
                    $("#myIframe").css("height", bottomBoxHeight + 'px');
                }
            });
        }
    },
