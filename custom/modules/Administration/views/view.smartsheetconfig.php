...
class AdministrationViewSmartSheetConfig extends SugarView {
    ...
    public function prepareRowData($sheet_id, $module, $return_val) {
        ...
        if (!empty($sheet_id) && !empty($module)) {
            ...
            //$saved_settings = $admin_settings->settings['smart_sheet_smart_sheet_' . $sheet_id];

            
            $saved_settings=  SmartSheetApiHelper::retrieveSettings($admin_settings->settings['smart_sheet_smart_sheet_' . $sheet_id]);
    ...
    public function getExtraOptionHtml($module, $field, $value, $row_count) {
        ...
        $extra_option_html = "<td style='visibility:' id='hidden_options_div_" . $row_count . "'>";
