...
class smartSheetSynchronizer {
    ...
    public function syncSugarToSmartSheet($optional_where = null, $enable_creation = false, &$return_flag = null) {
        ...
        if (!empty(json_decode($this->new_rows_for_update))) {
            ...
            //To check API reponse that record is updated successfully or not 
            if (isset($update_response->resultCode) && $update_response->resultCode == 0 && $update_response->message == 'SUCCESS') {
                $return_flag = $update_response->resultCode;
            }
        ...
        if ($enable_creation) {
            ...
            if (!empty(json_decode($this->new_rows_for_create))) {
                ...
                //To check API reponse that record is created successfully or not 
                if (isset($create_response->resultCode) && $create_response->resultCode == 0 && $create_response->message == 'SUCCESS') {
                    $return_flag = $create_response->resultCode;
                }
            }
        }
            ...
            if (empty($smart_sheet_result)) {
                ...
                foreach ($smart_sheet_result as $row_info_key => $row_info_value) {
                    ...
                    if (!empty($cam_bean_id)) {
                        ...
                        $this->setCallbackUpdateRows(array($row_info_value->id));
    ...
    private function createUpdateCamRecord($row, $smartsheet_to_sugar_fields) {
        ...
        if (!empty($row) && !empty($row->cells[1]->value)) {
            ...
            //replacement of retrieve_by_string_fields
            $query = new SugarQuery();
            $query->select('*'); // use id
            $query->from(BeanFactory::getBean('m_CAMS'));
            $query->where()->equals('job_number', trim($job_number));
            $result = $query->execute(); // use getOne function and this will return id as string
            if (!empty($result[0]['id'])) {
                $cam_bean = BeanFactory::getBean('m_CAMS', $result[0]['id']); // only use $result variable to fetch record
                //$GLOBALS['log']->fatal("cam bean id is " . $cam_bean->id);
            }
    ...
    private function mapSmartsheetCellWithSugarField($smartsheet_to_sugar_fields, $row, &$bean) {
        ...
        //$GLOBALS['log']->fatal('row is -->', $row);
        //$GLOBALS['log']->fatal('smartsheet_to_sugar_fields -->', $smartsheet_to_sugar_fields);

        foreach ($row->cells as $cell) {
            $field_name = $smartsheet_to_sugar_fields[$cell->columnId];
            if (!empty($field_name)) {

                //$GLOBALS['log']->fatal('field_name -->', $field_name);

                if (!empty($cell->value) && $cell->value != "1") {
                    //$GLOBALS['log']->fatal('if 1 and bean->field_name', $bean->$field_name);
                    //$GLOBALS['log']->fatal('if 2 and cell->value', $cell->value);
                    $bean->$field_name = $cell->value;
                }
                if (is_array($field_name) && $cell->value == "1") {
                    //$GLOBALS['log']->fatal('if 2 and bean->field_name', $bean->$field_name['name']);
                    //$GLOBALS['log']->fatal('if 2 and cell->value', $field_name['options']);
                    //$GLOBALS['log']->fatal('field_name', $field_name['name']);
                    //construction_stage
                    $name = $field_name['name'];
                    //$GLOBALS['log']->fatal('namee', $name);
                    $bean->$name = $field_name['options'];
                }
            }
        }

        //$GLOBALS['log']->fatal('construction_stage ->', $bean->construction_stage);
    }
    ...
    public function getSmartsheetFieldMappings($sheet_id) {
        if (!empty($sheet_id)) {
            ...
            $settings = SmartSheetApiHelper::retrieveSettings($admin_settings->settings['smart_sheet_smart_sheet_' . $sheet_id]);
            return $settings;
        }
        return '';
    }
    ...
    public function searchForRowIdInSmartsheet($job_number, $cells, $return_row_id = false) {
    ...
    private function sendNewSmartSheetRowsNotification() {
        ...
        if (strlen($this->new_smartsheet_records_email_body) > 0) {
            ...
            $primary_sender_email = SmartSheetApiHelper::retrieveSettings($admin->settings['smartsheet_email_id']);
    ...
    public function setCallbackUpdateRows($update_arr) {
        $this->call_back_update_rows_id = $update_arr;
    }
