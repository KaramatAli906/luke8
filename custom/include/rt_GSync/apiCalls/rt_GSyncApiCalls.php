...
class rt_GSyncApiCalls
{
    ...
    */
    function getPreferences($args)
    {
        ...
        if (!($bean == FALSE || $bean->deleted == 1)) {
            ...
            $data['multi_calendar'] = $bean->multi_calendar == '1' ? true : false;
            $data['calendar_sugar'] = $bean->calendar_sugar == '1' ? true : false;
            $data['calendar_google'] = $bean->calendar_google == '1' ? true : false;
        } else {
            $data['calendar_meetings'] = true;
            $data['calendar_calls'] = true;
            $data['calendar_tasks'] = true;
            $data['multi_calendar'] = false;
    ...
    */
    function setPreferences($args)
    {
        ...
        $bean->multi_calendar = $this->convertToBoolean($args['multi_calendar']);
        $bean->calendar_meetings = $this->convertToBoolean($args['calendar_meetings']);
        $bean->calendar_google = $this->convertToBoolean($args['calendar_google']);
        $bean->calendar_sugar = $this->convertToBoolean($args['calendar_sugar']);
    ...
    */
    function setUserConfig($args)
    {
        ...
        if ($this->dbFieldExists('users', 'enable_gsync')) {
            ...
            $isSugar78 = version_compare($sugar_version, '7.9', '<');
            if ($isSugar78) {
                ...
                $res1 = $conn->executeQuery($sql1);
                $res2 = $conn->executeQuery($sql2);
            }
