...
{
    private $path;
    private $function_mapping = array(
        'rt_is_dir' => 'is_dir',
        'rt_mkdir' => 'mkdir',
        'rt_touch' => 'touch',
        'rt_unlink' => 'unlink',
        'rt_clearstatcache' => 'clearstatcache',
        'rt_is_readable' => 'is_readable',
        'rt_filemtime' => 'filemtime',
        'rt_file_get_contents' => 'file_get_contents',
        'rt_file_put_contents' => 'file_put_contents',
    );
    ...
    private function isLocked($storageFile)
    {
        ...
        return SugarAutoloader::fileExists($storageFile . '.lock');
    }
    ...
    private function createLock($storageFile)
    {
        ...
        if (!$this->function_mapping['rt_is_dir']($storageDir)) {
            // @codeCoverageIgnoreStart
            //if (! @mkdir($storageDir, 0755, true)) {
            if (!@$this->function_mapping['rt_mkdir']($storageDir, 0755, true)) {
        ...
        @$this->function_mapping['rt_touch']($storageFile . '.lock');
    }
    ...
    private function removeLock($storageFile)
    {
        ...
        @$this->function_mapping['rt_unlink']($storageFile . '.lock');
    }
    ...
    private function waitForLock($storageFile)
    {
        ...
        do {
            ...
            //clearstatcache();
            $this->function_mapping['rt_clearstatcache']();
    ...
    public function getA($key, $expiration = false)
    {
        ...
        if (SugarAutoloader::fileExists($storageFile) && $this->function_mapping['rt_is_readable']($storageFile)) {
            $now = time();
            //if (! $expiration || (($mtime = @filemtime($storageFile)) !== false && ($now - $mtime) < $expiration)) {
            if (!$expiration || (($mtime = @$this->function_mapping['rt_filemtime']($storageFile)) !== false && ($now - $mtime) < $expiration)) {
                //if (($data = @file_get_contents($storageFile)) !== false) {
                if (($data = @$this->function_mapping['rt_file_get_contents']($storageFile)) !== false) {
    ...
    public function set($key, $value)
    {
        ...
        if (!@$this->function_mapping['rt_file_put_contents']($storageFile, $data)) {
            $this->removeLock($storageFile);
    ...
    public function delete($key)
    {
        ...
        //if (! @unlink($file)) {
        if (!@$this->function_mapping['rt_unlink']($file)) {
