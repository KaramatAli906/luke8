...
/*                    $phone_mobile_found_google = false; //will be true if phone_mobile is coming from google
    ...
    public static function retrieveUpdatedFromSugar($client, $assigned_user_id, $lastSync, $ids = array(), $user_email) {
        //$e = new Exception();
        //$GLOBALS['log']->fatal("Trace is:" . $e->getTraceAsString());
        global $db;
        $limit = 700;
        ...
        $q->select(array('users.id', 'users.gcontacts_offset_c', 'users.gcontacts_date_modified_c'));
        $q->where()->equals('id', $assigned_user_id);
        $res = $q->execute();
        if ($res[0]['gcontacts_offset_c'] === NULL) {
          $bean = new User();
          $bean->retrieve($assigned_user_id);
          $bean->gcontacts_date_modified_c='2013-01-01 01:01:01';
          $bean->gcontacts_offset_c=0;
          $bean->save();
        }
        $offset = $res[0]['gcontacts_offset_c'] ?: 0;
        $lastSync2 = $res[0]['gcontacts_date_modified_c'] ?: '2013-01-01 01:01:01';

        $sql = "SELECT main_table.id,midle_table.gcontact_id,main_table.deleted, main_table.date_modified FROM contacts main_table left join (SELECT * FROM contact_gcontact) AS midle_table ON midle_table.contact_id = main_table.id  AND midle_table.user_id = '" . $assigned_user_id . "' WHERE main_table.deleted = 0 AND main_table.date_modified >'" . $lastSync2 . "' AND main_table.id NOT IN ('" . implode("','", $ids) . "') ORDER BY main_table.date_modified ASC  LIMIT " . $offset . "," . $limit . ";";
        ...
        if ($res->num_rows < $limit) {
          ...
          // $GLOBALS['db']->updateParams(strtolower($module), $fieldDefs, array('gcontacts_offset_c' => 0,), array('gcontacts_date_modified_c' => $lastSync) );
          $sql = "UPDATE users_cstm SET gcontacts_date_modified_c ='" . $lastSync . "', gcontacts_offset_c=0 WHERE id_c='" . $assigned_user_id . "'";
          $db->query($sql);
        }
        else{
          $offset += $limit;
          $sql = "UPDATE users_cstm SET gcontacts_offset_c=" . $offset . " WHERE id_c='" . $assigned_user_id . "'";
          $db->query($sql);
          // $fieldDefs = $GLOBALS['dictionary'][rtrim($module,'s')]['fields'];
          // $GLOBALS['db']->updateParams(strtolower($module), $fieldDefs, array('gcontacts_offset_c' => $offset));
        ...
        while ($row = $db->fetchByAssoc($res)) {
            if ($row['deleted'] == '0') {
                ...
                //...............custom changes..........................//
                $select_sql = "select gcontact_id from contact_gcontact where user_id = '" . $assigned_user_id . "' and contact_id='" . $bean->id . "';";
                // $GLOBALS['log']->fatal("retrieveUpdatedFromSugar select sql: " . $select_sql);

                $select_result = $db->query($select_sql);
                $record = $db->fetchByAssoc($select_result);
                $bean->gcontact_id = $record['gcontact_id'];
                //$GLOBALS['log']->fatal("retrieveUpdatedFromSugar bean->gcontact_id: " . $bean->gcontact_id);
                //...............custom changes end..........................//
                $beans[] = $bean;
            } else if (!empty($row['gcontact_id'])){
                // $GLOBALS['log']->fatal("retrieveUpdatedFromSugar update for deltes contact: ");
...
//                $db->query($sql_update);
                $update_sql = "UPDATE contact_gcontact SET gcontact_id ='', deleted=1 WHERE contact_id='" . $row['id'] . "' and user_id ='" . $assigned_user_id . "';";
    ...
    public static function deleteFromGoogle($client, $ids = array(), $user_email) {
        ...
        foreach ($ids as $id) {
            try {
                ...
                //Sleep for 40 micro seconds to make up 40 seconds delay after 1000 records are synced to Google
                usleep( 40 * 1000 );
            } catch (Exception $ex) {
     ...
     * @access public
     */
    public static function sendUpdatedToGoogle($beans, $email_id, $googleClient, $user_id) {
        global $db;
                ...
                if ($bean->gcontact_id != '') {
                    ...
                    try {
                        ...
                        if ($e->getCode() == 404 || $e->getCode() == 403) { // not found or forbidden
                            ...
                            //$db->query($sql);
                            $update_sql = "UPDATE contact_gcontact SET gcontact_id =' ' WHERE contact_id='" . $bean->id . "' and user_id ='" . $user_id . "';";
                            $GLOBALS['log']->fatal("update sql for catach: " . $update_sql);
                            $db->query($update_sql);
                        }
                    }
                } else {
...
//                    
                    $contact_gcontactid = create_guid();
                    $insert_sql = "insert into contact_gcontact(id,date_modified,contact_id,gcontact_id,user_id) "
                            . "values('" . $contact_gcontactid . "',now(),'" . $bean->id . "', '" . $sub_response['entry']['id']['$t'] . "', '" . $user_id . "' );";
                    // $GLOBALS['log']->fatal("insert sql: " . $insert_sql);
                    $db->query($insert_sql);
//                   
                    //$sql = "UPDATE contact SET gcontact_id ='" . $sub_response['entry']['id']['$t'] . "' WHERE id='" . $bean->id . "';";
