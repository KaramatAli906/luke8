...
class MailChimpAdministration {
    ...
    private function _getFieldsMapping($data = array()) {
        ...
        $leads = $this->_clean($lead->field_defs, 'Leads');

        $prospect = BeanFactory::getBean('Prospects');
        $prospects = $this->_clean($prospect->field_defs, 'Prospects');
     ...
     * @return array
     */
    private function _clean($data = array(), $module = 'Contacts') {
        $filtered_data = array('' => 'Do Not Sync');
        if(!empty($data) && is_array($data) && count($data) > 0) {
            foreach($data as $key => $row){
                if(!in_array($row['type'], $this->exclude_field_types) && !in_array($row['name'], $this->exclude_field_names) && (empty($row['source']) || $row['source'] != 'non-db')) {
                    $filtered_data[$key] = translate($row['vname'], $module) . ' ('.$row['name'].')';;
                }
            }
        }
